...
```bash
npm exec @doist/react-compiler-tracker -- --overwrite
```
...
```bash
npm exec @doist/react-compiler-tracker -- --stage-record-file src/components/Button.tsx src/hooks/useData.ts
```
...
```bash
npm exec @doist/react-compiler-tracker -- --check-files src/components/Button.tsx src/hooks/useData.ts
```
...
```bash
npm exec @doist/react-compiler-tracker
```
...
```bash
npm exec @doist/react-compiler-tracker -- --check-files --show-errors src/components/Button.tsx
```
...
{
  "lint-staged": {
    "*.{js,jsx,ts,tsx}": "npm exec @doist/react-compiler-tracker -- --stage-record-file"
  }
}
```
...
- name: Check React Compiler violations
  run: |
    ...
    if [ -n "$FILES" ]; then
      npm exec @doist/react-compiler-tracker -- --check-files $FILES
    fi
```
...
if [ -n "$FILES" ]; then
  npm exec @doist/react-compiler-tracker -- --stage-record-file $FILES
fi
```
